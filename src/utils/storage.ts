...
function normalizeJob(raw: any): JobCard {
  ...
  return {
    ...
    salesOwnerName: raw.salesOwnerName ?? '',
    ...
    artworkAssignedDate: raw.artworkAssignedDate ?? '',
    artworkAssignedTo: raw.artworkAssignedTo ?? '',
    proofSharedDate: raw.proofSharedDate ?? '',
    proofSharedBy: raw.proofSharedBy ?? '',
    finalApprovalReceivedDate: raw.finalApprovalReceivedDate ?? '',
    finalApprovalClearedBy: raw.finalApprovalClearedBy ?? '',
    factoryReleaseDate: raw.factoryReleaseDate ?? '',
    factoryReleasedBy: raw.factoryReleasedBy ?? '',
    productionStartDate: raw.productionStartDate ?? '',
    productionStartedBy: raw.productionStartedBy ?? '',
    readyForDispatchDate: raw.readyForDispatchDate ?? '',
    readyForDispatchBy: raw.readyForDispatchBy ?? '',
    collectionOrDeliveryStatus: raw.collectionOrDeliveryStatus ?? 'Not Confirmed',
...
function normalizePaperRate(raw: any): PaperRate {
  return {
    ...
    useCases: Array.isArray(raw.useCases) && raw.useCases.length > 0
      ? raw.useCases
      : (raw.useCase ? [raw.useCase] : undefined),
...
function normalizeQuoteEstimate(raw: any): QuoteEstimate {
  ...
  return {
    ...
    linkedLeadId: raw.linkedLeadId ?? '',
    linkedLeadNumber: raw.linkedLeadNumber ?? '',
    salesOwnerName: raw.salesOwnerName ?? '',
    clientId: raw.clientId ?? '',
    clientName: raw.clientName ?? '',
    productId: raw.productId ?? '',
  ...
  try {
    ...
    return {
      ...
      products: (parsed.products ?? []).map((product: any) => ({
        ...
        pricingEnabled: Boolean(product.pricingEnabled),
        pricingSpec: product.pricingSpec ?? undefined,
      })),
      productPriceVersions: parsed.productPriceVersions ?? [],
      clientProductPrices: parsed.clientProductPrices ?? [],
      ...
      tooling: (parsed.tooling ?? []) as AppData['tooling'],
