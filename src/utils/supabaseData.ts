...
function mapPaperRate(row: any): PaperRate {
  return {
    ...
    chargePerTon: row.charge_per_ton == null ? undefined : Number(row.charge_per_ton),
...
function mapConsumableRate(row: any): ConsumableRate {
  return {
    id: row.id,
    name: row.name ?? '',
    supplierId: row.supplier_id ?? '',
    supplierName: row.supplier_name ?? '',
    productCode: row.product_code ?? undefined,
    category: (row.category ?? 'Other') as ConsumableCategory,
    unit: (row.unit ?? 'unit') as ConsumableUnit,
    publicLabel: row.public_label ?? undefined,
    costPerUnit: Number(row.cost_per_unit ?? 0),
    chargePerUnit: row.charge_per_unit == null ? undefined : Number(row.charge_per_unit),
    region: row.region ?? undefined,
...
export async function syncAppData(data: AppData): Promise<void> {
  await Promise.all([
    ...
    safeUpsert('paper_rates', data.paperRates.map((rate) => ({
      ...
      charge_per_ton: rate.chargePerTon ?? rate.pricePerTon,
      valid_from: rate.validFrom || null,
    ...
    safeUpsert('consumable_rates', (data.consumableRates ?? []).map((rate) => ({
      ...
      unit: rate.unit || 'unit',
      public_label: rate.publicLabel || null,
      cost_per_unit: rate.costPerUnit,
      charge_per_unit: rate.chargePerUnit ?? rate.costPerUnit,
      region: rate.region || null,
      valid_from: rate.validFrom || null,
      valid_to: rate.validTo || null,
      notes: rate.notes || null,
