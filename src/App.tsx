...
// Phase 128.1 — Unified material rate form. Replaces paper + consumable forms.
const createInitialMaterialRateForm = (): MaterialRateFormState => ({
  name: '',
  publicLabel: '',
  category: 'Paper',
  unit: '',
...
function App() {
  ...
  // Phase 128.1 — Unified MaterialRate state.
  const [materialRateForm, setMaterialRateForm] = useState(createInitialMaterialRateForm);
  const [materialRateEditingId, setMaterialRateEditingId] = useState<string | null>(null);
  const [materialRateMessage, setMaterialRateMessage] = useState('');
  const [materialRateFilters, setMaterialRateFilters] = useState<MaterialRateFilters>({ search: '', category: 'all', active: 'yes' });
  ...
  // Phase 128.1 — Unified paper-rate view. Calculator engine consumes
  // PaperRate-shape, so we adapt materialRates(category='Paper') into
  // PaperRate. Legacy paper_rates rows not yet copied into materialRates
  // are unioned in as a fallback so the transition period works.
  const paperRatesUnified = useMemo<PaperRate[]>(() => {
    const fromMaterials: PaperRate[] = (data.materialRates ?? [])
      ...
      .map((m) => ({
        ...
        gsm: m.gsm || '',
        pricePerTon: m.costPerUnit,
        chargePerTon: m.chargePerUnit,
        validFrom: m.validFrom,
        validTo: m.validTo,
        notes: m.notes,
        active: m.active,
      }));
    const migratedIds = new Set(fromMaterials.map((r) => r.id));
    const legacyOnly = data.paperRates.filter((r) => !migratedIds.has(r.id));
    return [...fromMaterials, ...legacyOnly];
  }, [data.materialRates, data.paperRates]);

  const paperRatesById = useMemo(() => new Map(paperRatesUnified.map((rate) => [rate.id, rate])), [paperRatesUnified]);
  ...
  // Phase 128.1 — Filtered unified material rates.
  const filteredMaterialRates = useMemo(() => (data.materialRates ?? []).filter((m) => {
    const matchesSearch = !materialRateFilters.search || [m.name, m.supplierName, m.publicLabel ?? '', m.category, m.productCode ?? ''].some((value) => matchesText(value, materialRateFilters.search));
    const matchesActive = materialRateFilters.active === 'all' || (materialRateFilters.active === 'yes' ? m.active : !m.active);
    const matchesCategory = materialRateFilters.category === 'all' || m.category === materialRateFilters.category;
    return matchesSearch && matchesActive && matchesCategory;
  }), [data.materialRates, materialRateFilters]);
  ...
  // Phase 128.1 — Unified material rate editor reset.
  function resetMaterialRateEditor() { setMaterialRateForm(createInitialMaterialRateForm()); setMaterialRateEditingId(null); setMaterialRateMessage(''); }
  ...
  function handleSavePaperRate() {
    ...
    const payload = {
      ...
      region: paperRateForm.region || undefined,
  ...
  // Phase 128.1 — Unified material rate save + edit. Single concept for
  // every cost master. Paper-specific fields (gsm/form/useCases) are only
  // captured when category === 'Paper'.
  function handleSaveMaterialRate() {
    if (!materialRateForm.publicLabel.trim() || !materialRateForm.category || !materialRateForm.unit) {
      setMaterialRateMessage('Public label, category, and unit are required.');
      return;
    }
    if (!materialRateForm.costPerUnit || !materialRateForm.chargePerUnit) {
      setMaterialRateMessage('Cost per unit and charge per unit are required.');
      return;
    }
    const linkedSupplier = materialRateForm.supplierId ? suppliersById.get(materialRateForm.supplierId) : undefined;
    const isPaper = materialRateForm.category === 'Paper';
    const payload: Omit<MaterialRate, 'id'> = {
      name: materialRateForm.name || materialRateForm.publicLabel,
      publicLabel: materialRateForm.publicLabel,
      category: materialRateForm.category,
      unit: materialRateForm.unit as MaterialRateUnit,
      supplierId: linkedSupplier?.id ?? '',
      supplierName: linkedSupplier?.name ?? '',
      productCode: materialRateForm.productCode || undefined,
      region: (materialRateForm.region || undefined) as PaperRegion | undefined,
      costPerUnit: Number(materialRateForm.costPerUnit),
      chargePerUnit: Number(materialRateForm.chargePerUnit),
      validFrom: materialRateForm.validFrom || undefined,
      validTo: materialRateForm.validTo || undefined,
      notes: materialRateForm.notes,
      active: materialRateForm.active,
      gsm: isPaper ? (materialRateForm.gsm || undefined) : undefined,
      paperType: isPaper ? (materialRateForm.paperType || undefined) : undefined,
      form: isPaper ? ((materialRateForm.form || undefined) as PaperForm | undefined) : undefined,
      useCases: isPaper && materialRateForm.useCases.length > 0 ? materialRateForm.useCases : undefined,
    };
    if (materialRateEditingId) {
      setData((current) => ({
        ...current,
        materialRates: (current.materialRates ?? []).map((m) => m.id === materialRateEditingId ? { ...m, ...payload } : m),
      }));
    } else {
      setData((current) => ({
        ...current,
        materialRates: [{ id: `mat-${Date.now()}`, ...payload }, ...(current.materialRates ?? [])],
      }));
    }
    resetMaterialRateEditor();
  }

  function editMaterialRate(m: MaterialRate) {
    setMaterialRateEditingId(m.id);
    setMaterialRateForm({
      name: m.name,
      publicLabel: m.publicLabel || '',
      category: m.category,
      ...
      chargePerUnit: String(m.chargePerUnit ?? m.costPerUnit),
      validFrom: m.validFrom || '',
      validTo: m.validTo || '',
      notes: m.notes,
      active: m.active,
  ...
  function handleApproveProductPrice(productId: string, note: string) {
    ...
    const refs = { pricingTiers: data.pricingTiers, paperRates: paperRatesUnified, costProfiles: data.costProfiles };
  ...
  return (
    ...
    >
      ...
      {view === 'costInputs' && canManageCostInputs && (
        <CostInputsPage
          suppliers={data.suppliers}
          // Phase 128.1 — Unified MaterialRate plumbing.
          materialRates={data.materialRates ?? []}
          materialRateForm={materialRateForm}
          setMaterialRateForm={setMaterialRateForm}
          materialRateEditingId={materialRateEditingId}
          materialRateMessage={materialRateMessage}
          onSaveMaterialRate={handleSaveMaterialRate}
          onResetMaterialRate={resetMaterialRateEditor}
          materialRateFilters={materialRateFilters}
          setMaterialRateFilters={setMaterialRateFilters}
          filteredMaterialRates={filteredMaterialRates}
          onEditMaterialRate={editMaterialRate}
          costProfiles={data.costProfiles}
